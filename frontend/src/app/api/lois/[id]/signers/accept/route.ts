...
/**
 * POST /api/lois/[id]/signers/accept
 * Set status=accepted, soft_commitment_at=now()
 */
export async function POST(
  ...
  try {
    ...
    // Verifica che lo status sia valido per l'accettazione
    if (signer.status !== "invited") {
      return NextResponse.json(
        { error: `Cannot accept signer with status: ${signer.status}. Expected: invited` },
        { status: 400 }
      );
    }
    ...
    console.error("Error in POST /api/lois/[id]/signers/accept:", err);
